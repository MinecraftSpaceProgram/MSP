...
public class SpaceSkyRenderer {
  ...
  private final TextureManager TEXTURE_MANAGER = Minecraft.getInstance().textureManager;
  ...
  public void render(float partialTicks, MatrixStack matrixStackIn, ClientWorld world, Minecraft mc)
  {
    ...
    float[] afloat = world.func_239132_a_().func_230492_a_(1.0F, partialTicks);
    ...
    matrixStackIn.rotate(Vector3f.XP.rotationDegrees(1.0F * 360.0F));/*world.getCelestialAngle(partialTicks) * 360.0F*/ // world.func_242415_f(partialTicks)
    Matrix4f matrix4f1 = matrixStackIn.getLast().getMatrix();
    float f12 = 30.0F;
    TEXTURE_MANAGER.bindTexture(SUN_TEXTURES);
    ...
    WorldVertexBufferUploader.draw(bufferbuilder);
    f12 = 20.0F;
    TEXTURE_MANAGER.bindTexture(MOON_PHASES_TEXTURES);
    int k = 1; // world.getMoonPhase();
    int l = k % 4;
    int i1 = k / 4 % 2;
    ...
    double d0 = mc.player.getEyePosition(partialTicks).y - 30/*world.getHorizonHeight()*/; // TODO
    MSP.LOGGER.debug(d0);
    if (d0 < 0.0D)
    {
      matrixStackIn.push();
