...
public class SeatEntity extends Entity {
  ...
  public SeatEntity(EntityType<?> type, World worldIn) {
    super(type, worldIn);
  }

  private SeatEntity(World world, BlockPos source, double yOffset) {
    this(MSPEntityTypes.SEAT_TYPE.get(), world);
    this.source = source;
    ...
    if (!this.world.isRemote) {
      if (this.getPassengers().isEmpty() || this.world.isAirBlock(source)) {
        ...
        world.updateComparatorOutputLevel(
            getOnPosition(), world.getBlockState(getOnPosition()).getBlock());
      }
    }
  }
  ...
  public static ActionResultType create(
      World world, BlockPos pos, double yOffset, PlayerEntity player) {
    if (!world.isRemote) {
